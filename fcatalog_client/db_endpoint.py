...
class MsgTypes:
    ...
    RESPONSE_SIMILARS = 3
    ADD_STRUCTURE = 4
    REQUEST_STRUCT_NAMES = 5
    RESPONSE_STRUCT_NAMES = 6
    REQUEST_STRUCT = 7
    RESPONSE_STRUCT = 8
    SYN = 9
    ACK = 10
    REQUEST_FUNC_NAMES = 11
    RESPONSE_FUNC_NAMES = 12
...
def build_msg_request_func_names():
    msg = dword_pack(MsgTypes.REQUEST_FUNC_NAMES, "")
    return msg
...
def build_msg_syn():
    msg = dword_pack(MsgTypes.SYN, '')
    return msg

def build_msg_add_struct(struct_name,struct_dump):
    inner_msg = ''.join([struct_name+"\x00", struct_dump])
    msg = dword_pack(MsgTypes.ADD_STRUCTURE, inner_msg)
    return msg

def build_msg_request_struct_names():
    msg = dword_pack(MsgTypes.REQUEST_STRUCT_NAMES, "")
    return msg

def build_msg_request_struct(struct_name):
    msg = dword_pack(MsgTypes.REQUEST_STRUCT, struct_name)
    return msg
...
class DBEndpoint(object):
    ...
    def request_func_names(self):
        self._frame_endpoint.send_frame(\
            build_msg_request_func_names()\
            )

    def response_func_names(self):
        ...
        if msg_type != MsgTypes.RESPONSE_FUNC_NAMES:
            raise DBEndpointError('Invalid msg_type returned from server')

        struct_list = msg.split('\x00')
        result = []
    ...
    def add_structure(self,struct_name,struct_dump):
        self._frame_endpoint.send_frame(\
            build_msg_add_struct(struct_name,struct_dump) \
            )

    def request_struct_names(self):
        self._frame_endpoint.send_frame(\
            build_msg_request_struct_names()\
            )

    def response_struct_names(self):
        # Wait for result from RequestSimilars query:
        frame = self._frame_endpoint.recv_frame()
        if frame is None:
            raise DBEndpointError('Remote host has closed the connection')
        ...
        msg_type, msg = dword_unpack(frame)
        if msg_type != MsgTypes.RESPONSE_STRUCT_NAMES:
            raise DBEndpointError('Invalid msg_type returned from server')
    ...
    def request_struct(self, name):
        self._frame_endpoint.send_frame(\
            build_msg_request_struct(name)\
            )
    ...
    def response_struct(self):
        ...
        msg_type, msg = dword_unpack(frame)
        if msg_type != MsgTypes.RESPONSE_STRUCT:
            raise DBEndpointError('Invalid msg_type returned from server')
